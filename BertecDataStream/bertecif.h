...
#define BERTEC_LIBRARY_VERSION   (0x214)

        /** This defines how many channels and devices that the DLL can support. Currently up to 4 devices with 32 channels each. */
#define BERTEC_MAX_CHANNELS      (32)
#define BERTEC_MAX_DEVICES       (4)
#define BERTEC_MAX_NAME_LENGTH   (16)
...
#pragma pack(8)
    ...
    struct bertec_DeviceInfo
    {
        ...
        char  serial[BERTEC_MAX_NAME_LENGTH + 1];       /* the serial # of the device */
        char  model[BERTEC_MAX_NAME_LENGTH + 1];        /* the model # of the device */
        ...
        bool  hasAuxSyncPins;    /* if set, then the device has full control over the aux and sync pins. */
        ...
        char  channelNames[BERTEC_MAX_CHANNELS][BERTEC_MAX_NAME_LENGTH + 1];  /* the channel names from the device's eprom, null terminated */
    };
...
#pragma pack(pop)
    ...
    /** If not using callbacks, calling this will read one sample from the buffered data and return how many samples remain including this one.
